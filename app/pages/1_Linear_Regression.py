...
st.title("📈 Interactive Linear Regression Lab")

st.markdown("Use the controls on the left to see how the Line of Best Fit changes based on the data points!")

# Sidebar controls
n_points = st.sidebar.slider("Number of Points", 10, 200, 50)
noise = st.sidebar.slider("Noise Level", 0.0, 20.0, 5.0)
slope = st.sidebar.slider("Slope", 1.0, 10.0, 4.5)

# Generate synthetic data
x = np.linspace(0, 10, n_points)
y = slope * x + 12 + (np.random.randn(n_points) * noise)
...
ax.scatter(x, y, color='blue', alpha=0.5, label='Synthetic Student Data')
